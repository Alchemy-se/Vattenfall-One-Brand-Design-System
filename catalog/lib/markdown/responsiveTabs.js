...
import ResponsiveTabs2 from './ResponsiveTabs2'
...
export default class ResponsiveTabs extends Component {
    ...
    switch (name) {
      case 'Phone': {
        return (
          <svg viewBox='0 0 2 2'>
            <rect
              className='catalog-11861ax-Preview-className'
              width='0.5622188905547226'
              height='1'
              x='0.7188905547226387'
              y='0.5'
            ></rect>
          </svg>
        )
  ...
  renderTabs = () => {
    return responsiveSizes.map((item, index) => {
      const tabClasses =
        item.name === this.state.selectedTab ? styles.tabSelected : styles.tab
      return (
        <div
          className={tabClasses}
          onClick={() => this.selectTab(item.name, item.width)}
          key={index}
        >
  ...
  getStyle() {
    if (this.iframeWrapper && this.iframeWrapper.current) {
      const scale = Math.min(
        1,
        this.iframeWrapper.current.offsetWidth / this.state.selectedWidth
      )
